...
async function getRequests(query = {}, requestType) {
  let finalUrl =
    API_BASE_URL +
    (nextLink || '/requests' + getQueryParamsString(query, requestType));
  ...
  function generateRequestContent() {
    return createElementFromMap({
      ...
      child: [
        createElementFromMap({
          tagName: 'p',
          textContent: type === 'ONBOARDING' ? reason : message || 'N/A',
        }),
        createElementFromMap({
          tagName: 'div',
...
async function renderRequestCards(queries = {}) {
  ...
  try {
    ...
    requestResponse =
      currentReqType === OOO_REQUEST_TYPE
        ? await getRequests(queries, OOO_REQUEST_TYPE)
        : currentReqType === ONBOARDING_EXTENSION_REQUEST_TYPE
        ? await getRequests(queries, ONBOARDING_EXTENSION_REQUEST_TYPE)
        : await getRequests(queries, EXTENSION_REQUEST_TYPE);
