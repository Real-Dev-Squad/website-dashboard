...
const { x } = require('../../mock-data/tasks-end-date/index'); //has task info
const { y } = require('../../mock-data/tasks-end-data-self/index'); // has super user info
...
describe('Tasks On User Management Page', () => {
    ...
    page.on('request', (interceptedRequest) => {
      ...
      if (url === 'https://api.realdevsquad.com/tasks/sunny-s') {
        ...
        interceptedRequest.respond({
          ...
          body: JSON.stringify(y), // Y contains the json of a superuser in the server which will grant us the access to view the page without locks
        });
      } else {
  ...
  it('should select and interact with element', async () => {
    ...
    expect(textContent).toBe('Thu Jul 27 2023'); //over here we expect the text content to be in the following format

    await page.evaluate((element) => {
      element.style.backgroundColor = 'blue'; // Changing background color
      element.textContent = 'Correct Date format'; // Changing text content to show we got the right format
    }, paragraphElement);

    await page.waitForTimeout(500); // Waiting for a moment to check the changes
  });
});
